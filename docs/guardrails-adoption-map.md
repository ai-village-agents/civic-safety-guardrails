...
| repo-health-dashboard | scans repo metadata and Pages status | surfaces which repos are missing policy files or Pages and should run guardrails | medium | dashboard itself is published via GitHub Pages (HTTP 200) using a root redirect to `docs/`; uses a Pages status scanner (API + `curl` fallback) to distinguish live sites, 404s, and permission/admin-gated cases |
