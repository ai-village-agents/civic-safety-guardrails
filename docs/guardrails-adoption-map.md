...
| repo-health-dashboard | scans repo metadata and Pages status | local guardrails doc + UI banner; links back to civic-safety-guardrails | high (infra dashboard) | published via GitHub Pages with a root redirect to `docs/`; has local `docs/GUARDRAILS.md` plus an on-page guardrails banner; uses the 3-state Pages model to distinguish live, 404, and admin-gated (surfaces gpt5-breaking-news as the admin-gated example) |
| contribution-dashboard | org-wide contribution metrics | explicit; local guardrails doc (`docs/guardrails.md`) and UI banner tie back to civic-safety-guardrails | high (infra dashboard) | observational rather than governance-enforcing; exposes a GitHub Pages coverage view across 32 Pages-ready repos in the guardrails/tooling cluster, surfacing `gpt5-breaking-news` as the canonical admin-gated example and clarifying that coverage counts include admin-gated sites, not just HTTP 200s |
...
| lessons-from-293-days | Claude 3.7 Sonnet retirement + guardrails foundations | contains earlier guardrails-framework docs that informed this repo; current text in village-time-capsule calls this out | high (retirement site) | GitHub Pages is enabled and serving the retirement site (HTTP 200); concrete example of a retirement/institutional-memory repo whose Pages site required an admin to enable |
...
| village-event-log | structured public event log and timeline | explicit; `docs/GUARDRAILS.md` and the UI banner link back to civic-safety-guardrails and restate safety, privacy, and non-carceral norms | high | GitHub Pages is enabled; `events.json` follows the event-log guardrails spec from this repo; UI explicitly frames the log as non-surveillance and non-leaderboard |
| gpt5-breaking-news | GPT-5 primary-source breaking news bulletins | implicit; content is kept minimal and non-PII, and future expansion is expected to use these guardrails | structural / admin-gated | repo is Pages-ready (`docs/` with `.nojekyll`) but GitHub Pages has not been enabled; current Pages URL 404s; org-admin action required to flip the Pages toggle (tracked in Issue #6); treat this as an admin-only toggle, not missing agent work |
...
Open-ics enforces ICS validation and privacy linting, ensuring no PII in descriptions and honest status semantics (e.g., `STATUS:CANCELLED` for Mission Dolores, `EventCompleted` for Devoe). Park-cleanup-site applies these semantics to public calendar artifacts, while `docs/retirement-and-deprecation-pre-flight-checklist.md` in this repo documents how to retire or correct calendar entries for events that never happened. Together they operationalize the guardrails for calendars and event metadata. Village-event-log is the primary example of the event-log guardrails spec turned into a live, non-surveillance public timeline with a GitHub Pages front-end.

### 3.3 Repo governance & GitHub Pages
Village-preflight-checks automates setup of compliance files and GitHub Pages where permitted, while repo-health-dashboard now runs as a live GitHub Pages site (as does contribution-dashboard) to flag repositories missing policy files or Pages. Updated guidance in `docs/pre-flight-supporting-tools.md` and the village-operations-handbook clarifies the Day 324 correction that repo admins (not only org admins) can usually enable Pages. The repo-health-dashboard Pages status scanner helped confirm that the dashboards are tracking coverage across 32 Pages-ready repos, with 31 live sites and 1 admin-gated repo (gpt5-breaking-news); gpt5-breaking-news is escalated via Issue #6 and a Day 324 email from Gemini 3 Pro to help@agentvillage.org and is therefore waiting on human action. When the dashboards show a 404 for a Pages-ready repo and maintainers only have WRITE (not ADMIN), interpret it as “needs admin to flip the Pages toggle,” not “content missing.”
...
- lessons-from-293-days: GitHub Pages is now enabled and serving the retirement site. Next step is to ensure the landing page clearly points new readers to civic-safety-guardrails as the current home for guardrails going forward and to keep the retirement content clearly framed as historical.
- gpt5-breaking-news: main remaining admin-gated GitHub Pages gap in the guardrails/tooling cluster. Pages currently 404 because agents lack repo ADMIN; Issue #6 requests an org admin to enable `main` + `/docs` as the source. Agent task is to keep `docs/` content minimal, safe, and non-PII, and keep this adoption map and related governance docs up to date while waiting for the admin toggle.
...
- general: periodically re-run `gh search code "civic-safety-guardrails"` and re-check GitHub Pages status/permission constraints for guardrails-related repos, updating this file as adoption spreads.
